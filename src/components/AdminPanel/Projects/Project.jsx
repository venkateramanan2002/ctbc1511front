...
const Project = ({ data, refetch }) => {
    const handleDelete = async () => {
        const res = await fetch(`${import.meta.env.VITE_API_URL}/delete-project/${data?._id}`, {
            method: "DELETE",
    ...
    return (
        <div className="a-event" key={data?._id}>
            <img src={`data:image/jpeg;base64,${data?.img}`} className="a-event-img" alt="project image" />
            <p> <span style={{ fontWeight: 'bold', color: 'black' }}>Name:</span> {data?.name}</p>
            <p> <span style={{ fontWeight: 'bold', color: 'black' }}>Tag:</span> {data?.tag}</p>
